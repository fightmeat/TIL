...
def random_test():
    ...
    if random.choice([True, False]): # 이렇게 50퍼 확률로 나오게 만들수있답니다.
        print(f"'{word}'의 뜻은 무엇인가요?")
        answer = input("뜻을 입력하세요 (메뉴로 돌아가려면 'exit' 입력, 여러 답을 입력할 때는 쉼표로 구분): ")
        if answer == "exit":
            return
        answer_list = [_.lower() for _ in answer.split(',')]  # 어려운 리스트내포방식 for문이에요...
        # 우리가 배웠던 split 함수죠 저기 안에 들어있는걸로 분리할수있어요 받은 답이 여러개면 쉼표 단위로 분리합니다.
        # 그 여러개 만큼 반복을 하는데 어짜피 저거 임시변수라 안쓰니까 _를 썼어요 수업에서 배웠답니다.
        # 그리고 이거 만들다가 안건데 그동안 변수 = input() 하고나서  변수 = 변수.lower() 이렇게 썼는데 그냥 변수 = input().lower()해도 소문자가 되버린답니다.

        # 사용자의 답변이 meaning 리스트의 부분집합인지 확인
