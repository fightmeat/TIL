...
def random_test():
    if not vocabulary:  # vocabulary가 비어있으면 테스트할 단어가 없다는 메시지 출력
        print("테스트할 단어가 없습니다. 먼저 단어를 추가해주세요.")
        return

    word, meanings = random.choice(list(vocabulary.items()))  # 랜덤하게 단어와 뜻 선택
    selected_meaning = random.choice(meanings)  # 여러 뜻 중 하나를 랜덤하게 선택

    # 50% 확률로 문제 유형 결정 (단어의 뜻 묻기 vs 뜻 보고 단어 맞추기)
    if random.choice([True, False]):
        # 단어의 뜻을 물어보는 문제
        print(f"'{word}'의 뜻은 무엇인가요?")
        answer = input("뜻을 입력하세요 (메뉴로 돌아가려면 'exit' 입력): ")
        if answer == "exit":
            return
        if answer in meanings:  # 사용자의 응답이 meanings 리스트 내의 값 중 하나와 일치하는지 확인
            print("정답입니다!")
        else:
            print(f"틀렸습니다. '{word}'의 뜻은 '{', '.join(meanings)}' 입니다.")
    else:
        # 뜻을 보고 단어를 맞추는 문제
